...
def get_winner(game_folder_path):
...
def place_winner(screen, winner_id, winner_imgs, img_id):
    print(winner_id, winner_imgs, img_id)
    screen.blit(winner_imgs[img_id], (player_positions[winner_id][0] - 45, player_positions[winner_id][1] - 45))
...
winner_circles = [pygame.image.load("assets/White_circle.png"), pygame.image.load("assets/Green_circle.png")]
...
while True:
    ...
    if newgame_counter >= 0:
        ...
        if curr_winner is None:
            curr_winner = get_winner(game_folder)
        print(f"Winner: Player {curr_winner}")
        if newgame_counter == 0:
            newgame = True
            curr_winner = None
        else:
            place_winner(screen, curr_winner, winner_circles, w_img_id)
        newgame_counter -= 1
